...
// Solo la web propia y el dev local pueden llamar al API desde navegador.
// Peticiones sin Origin (curl, server-to-server) se permiten: Turnstile las frena igual.
const ALLOWED_ORIGINS = ['https://andreumatic.com', 'https://www.andreumatic.com', 'http://localhost:3000', 'http://127.0.0.1:3000'];

function cors(origin) {
  return {
    'Access-Control-Allow-Origin': !origin || ALLOWED_ORIGINS.includes(origin) ? (origin || ALLOWED_ORIGINS[0]) : ALLOWED_ORIGINS[0],
...
function json(payload, status = 200, origin = '') {
  return Response.json(payload, { status, headers: cors(origin) });
}
...
export async function onRequestPost({ request, env }) {
  ...
  const j = (payload, status = 200) => json(payload, status, origin);
  if (origin && !ALLOWED_ORIGINS.includes(origin)) {
    return j({ ok: false, message: 'Origen no permitido.' }, 403);
  }
  const ip = request.headers.get('cf-connecting-ip') || 'unknown';
  if (rateLimited(ip)) {
    return j({ ok: false, message: 'Demasiados intentos. Prueba en unos minutos.' }, 429);
  }
  ...
  if (secret) {
    if (!token) {
      return j({ ok: false, message: 'Verifica que no eres un robot e inténtalo de nuevo.' }, 400);
    }
    try {
      ...
      return j({ ok: false, message: 'No se pudo verificar la solicitud.' }, 500);
    }
  }

  // Saneado anti-inyección: sin saltos de línea en campos que van al asunto
  const nombre = String(body.nombre || '').trim().replace(/[\r\n]+/g, ' ').slice(0, 120);
  ...
  if (email && !emailOk) {
    return j({ ok: false, message: 'Revisa el email indicado.' }, 400);
  }

  if (!nombre || !telefono || !mensaje) {
    return j({ ok: false, message: 'Rellena tu nombre, tu teléfono y tu caso antes de enviar.' }, 400);
  }
  ...
  if (!/^[a-zA-ZáéíóúÁÉÍÓÚñÑüÜ\s'-.]{2,120}$/.test(nombre)) {
    return j({ ok: false, message: 'Revisa el nombre indicado.' }, 400);
  }
  if (/(https?:\/\/|www\.)/i.test(mensaje) && mensaje.length < 40) {
    return j({ ok: false, message: 'Describe tu caso con tus palabras, sin enlaces.' }, 400);
  }

  const apiKey = env.RESEND_API_KEY;
  if (!apiKey) {
    return j({ ok: true, message: 'Solicitud recibida (modo prueba: sin RESEND_API_KEY no se envía email).', delivery: 'modo-prueba' });
  }
  ...
  try {
    ...
    if (emailOk) payload.reply_to = email; /* responder va directo al cliente */
    ...
    if (!res.ok) {
      ...
      return j({ ok: false, message: 'No se pudo enviar la solicitud.' }, 502);
    }
    return j({ ok: true, message: 'Solicitud enviada correctamente.', delivery: 'email-enviado' });
  } catch (e) {
    console.error('Contact error:', e);
    return j({ ok: false, message: 'No se pudo enviar la solicitud.' }, 500);
  }
}
